...
const Projectitem = ({title, backgroundImg, projectUrl}) => {
  return (
    ...
    <div className='hidden group-hover:block absolute top-[58%] left-[50%] translater-x-[-50%] translate-y-[-50%]'>
        <h3 className='text-2xl text-[#2a4dad] tracking-widest text-center '>{title}</h3>
        <p className='pb-4 pt-2 text-white text-center'></p>
        <a href={projectUrl}>
            <p className='text-center py-3 rounded-lg bg-white text-[#393b3d] font-bold text-lg cursor-pointer'>More Info</p>
        </a>
    </div>
</div>
  )
